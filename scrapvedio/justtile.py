...
if __name__ == '__main__':
    ...
    global songdicts
    songdicts = {}
    songdictfile = '%s/titleinfo.json' % (basePath)
    try:
    ...
    while True:
        num = num + 1
        url = 'http://www.proedu.com.cn/web/shareVideo/index.action?id=%s&ajax=1' % num
        logging.info (url)
        main(url)
        if num > allnu:
            break
    ...
    with open('%s/titleinfo.json' % basePath,'w') as f:
        f.write(line)
    #logging.info(aresults)


        with open('%s/biaoti.csv' % (basePath), 'w') as f:
            for k,v in songdicts.items():
                n1strs = []
...
"""
    "2-6 利用网络直接复制安装额外域控制器": {
        "name": "2-6 利用网络直接复制安装额外域控制器",
        "url": "http://www.proedu.com.cn/web/shareVideo/index.action?id=1067543&ajax=1",
        "mp4url": "http://vod0vwkapu4.vod.126.net/vod0vwkapu4/bBrpcfCN_2753409424_shd.mp4",
        "localpath": "/export/lichengshuang/studygo/mp4//2-6 利用网络直接复制安装额外域控制器.mp4"
